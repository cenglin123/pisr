...
def _check_structure() -> list[dict[str, Any]]:
    ...
    structure_dir = ROOT / "docs"
    indexed_paths: set[str] = set()

    for target, _label, _lineno in index_entries:
        resolved = _resolve(target, structure_dir)
        try:
            indexed_paths.add(str(resolved.relative_to(ROOT)).replace("\\", "/"))
        except ValueError:
            # Links outside the repository are handled as missing below.
            pass
    ...
    if docs_dir.is_dir():
        for f in sorted(docs_dir.rglob("*.md")):
            ...
            if rel == "docs/STRUCTURE.md":
                continue
            if any(rel == indexed or rel.startswith(indexed.rstrip("/") + "/")
